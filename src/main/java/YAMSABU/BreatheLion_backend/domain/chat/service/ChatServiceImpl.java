...
public class ChatServiceImpl implements ChatService{
    ...
    public ChatStartResponseDTO startChatting(ChatRequestDTO chatRequestDTO){
        ...
        Chat chatRequest = ChatConverter.requestToChat(chatRequestDTO, record);
        chatRepository.save(chatRequest);

        // AI 답변 생성
        String aiAnswer = aiService.ChatAnswer(chatRequest.getMessage());
        Chat answerChat = ChatConverter.anwerToChat(aiAnswer, record);
        chatRepository.save(answerChat);

        // DTO 생성 후 반환
        return ChatConverter.toChatStartResponseDTO(record.getId(), answerChat);
    }
    ...
    public ChatAnswerDTO attachChatting(ChatWithEvidenceDTO chatWithEvidenceDTO){
        ...
        Chat chatRequest = ChatConverter.requestToChat(ChatRequestDTO.builder().message(text).build(), record);
    ...
    public ChatEndResponseDTO endChatting(ChatEndRequestDTO chatEndRequestDTO){
        Long recordId = chatEndRequestDTO.getRecordId();
        Record record = recordRepository.findById(recordId)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "레코드가 존재하지 않습니다."));
